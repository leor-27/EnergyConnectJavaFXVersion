...
public class HomepageController extends NavController {
    ...
    private void loadOnAirProgram() {
        ...
        for (Program p : programs) {
            LocalTime start = LocalTime.parse(p.startTime.substring(0, 5));
            LocalTime end   = LocalTime.parse(p.endTime.substring(0, 5));
        ...
        onAirLabel.setText(
                onAir != null ? "On Air: " + onAir.title : "No Active Program"
        );
    }
    ...
    private void loadFeaturedPrograms() {
        ...
        for (Program p : programs) {
            ...
            Label title = new Label(p.title + " | " + p.days);
            title.setStyle("-fx-font-weight: 700; -fx-font-size: 20");

            header.getChildren().addAll(mic, title);

            Label time = new Label("Time: " + p.startTime + " - " + p.endTime);
            time.setStyle("-fx-font-size: 18");

            Label hosts = new Label("Hosts: " + p.hosts);
    ...
    private void loadFeaturedNews() {
        ...
        """);
            ...
            ImageView img = createCoverImage(n.imagePath, 325, 300);
            img.setCursor(javafx.scene.Cursor.HAND);
            img.setOnMouseClicked(e -> {
                if (n.sourceUrl != null && !n.sourceUrl.isBlank()) {
                    HelloApplication.getHostServicesInstance().showDocument(n.sourceUrl);
                }
            });
            img.setStyle("-fx-background-radius: 10;");
            img.setSmooth(true);

            Label headline = new Label(n.headline);
            headline.setWrapText(true);
            headline.setStyle("-fx-font-size: 20; -fx-font-weight: 700;");

            Label meta = new Label(
                    n.organization + " | " + formatNewsDate(n.datePosted)
            );
            meta.setStyle("-fx-font-size: 14; -fx-text-fill: #555;");

            Label author = new Label("By " + n.author);
            author.setStyle("-fx-font-size: 14;");

            Label summary = new Label(n.summary);
    ...
    private String formatNewsDate(String dateTime) {
        LocalDateTime posted = LocalDateTime.parse(
                dateTime.replace(" ", "T")
        );
