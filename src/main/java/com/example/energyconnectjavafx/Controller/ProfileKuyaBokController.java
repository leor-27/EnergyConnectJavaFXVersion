package energyfm.energyconnect.energyconnectjavafxabout.Controllers;

import javafx.fxml.Initializable;
import javafx.scene.image.Image;
import javafx.scene.image.ImageView;
import javafx.scene.text.Text;
import java.net.URL;
import java.util.ResourceBundle;

public class ProfileKuyaBokController extends ProfileMainController implements Initializable {

   public Text djNicknameText;
   public Text realNameText;
   public ImageView djPicture;
   public ImageView leftArrow;
   public ImageView rightArrow;

   @Override
   public void initialize(URL location, ResourceBundle resources) {
      // Show DJ's names
      if (djNicknameText != null) {
         djNicknameText.setText("KUYA BOK");
      }
      if (realNameText != null) {
         realNameText.setText("Bok Millare");
      }

      // Make arrows work
      if (leftArrow != null) {
         leftArrow.setOnMouseClicked(event -> {
            try {
               goToPreviousDJ(event);
            } catch (Exception e) {
               e.printStackTrace();
            }
         });
      }

      if (rightArrow != null) {
         rightArrow.setOnMouseClicked(event -> {
            try {
               goToNextDJ(event);
            } catch (Exception e) {
               e.printStackTrace();
            }
         });
      }

      // Show DJ's picture
      if (djPicture != null) {
         try {
            Image image = new Image(getClass().getResourceAsStream("/images/kuya_bok.png"));
            djPicture.setImage(image);
         } catch (Exception e) {
            e.printStackTrace();
         }
      }
   }
}